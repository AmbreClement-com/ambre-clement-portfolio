'use client';
/* eslint-disable */
// Vendored depuis React Bits (Splash Cursor — simulation de fluide WebGL,
// d'après le WebGL-Fluid-Simulation de Pavel Dobryakov, MIT). Adapté pour
// Ambre Clément : teinte BLANCHE + mix-blend-difference (inversion des couleurs
// de la photo) au lieu d'une couleur, garde notre cadre par-dessus.
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Pas de fluide au toucher / reduced-motion (effet décoratif lourd).
    if (
    ...
    // En mode CONTAINED le canvas n'occupe pas tout l'écran : on convertit les
    // coordonnées clientX/Y en coordonnées RELATIVES au canvas (sinon les splats
    // tombent hors-cadre). En plein écran (canvas en 0,0) ça ne change rien.
    const relX = (cx: number) =>
      CONTAINED ? cx - canvas.getBoundingClientRect().left : cx;
    const relY = (cy: number) =>
      CONTAINED ? cy - canvas.getBoundingClientRect().top : cy;

    window.addEventListener('mousedown', e => {
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(relX(e.clientX));
      const posY = scaleByPixelRatio(relY(e.clientY));
  ...
  return (
    <div
      ...
      style={{
        ...
        top: 0,
        left: 0,
        zIndex: CONTAINED ? 1 : 40,
        pointerEvents: 'none',
        width: '100%',
        height: '100%',
        opacity: OPACITY,
        // Le fluide blanc INVERSE les couleurs de ce qu'il y a dessous.
